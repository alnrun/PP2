...
# Coin types: (color, value, radius, weight)
# Higher weight = more likely to spawn
COIN_TYPES = [
    (YELLOW, 1, 10, 60),  # common   - 1 point,  small
    (CYAN,   3, 13, 30),  # uncommon - 3 points, medium
    (ORANGE, 5, 16, 10),  # rare     - 5 points, large
]
...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        weights = [c[3] for c in COIN_TYPES]
        coin_color, self.value, radius, _ = random.choices(COIN_TYPES, weights=weights)[0]

        # Draw coin as a colored circle
        size = radius * 2
        self.image = pygame.Surface((size, size), pygame.SRCALPHA)
        pygame.draw.circle(self.image, coin_color, (radius, radius), radius)
        self.rect = self.image.get_rect()
        self.rect.center = (random.randint(40, SCREEN_WIDTH - 40), 0)
...
while True:
    ...
    # Draw road and grass
    screen.fill(GRAY)
    pygame.draw.rect(screen, GREEN, (0, 0, 50, SCREEN_HEIGHT))
    pygame.draw.rect(screen, GREEN, (SCREEN_WIDTH - 50, 0, 50, SCREEN_HEIGHT))
    ...
    # Collect coins - add each coin's value to total
    for coin in pygame.sprite.spritecollide(P1, coins_group, True):
        COINS += coin.value
        # Speed up every 5 coins collected
        if COINS % 5 == 0 and SPEED < 15:
            SPEED += 1
